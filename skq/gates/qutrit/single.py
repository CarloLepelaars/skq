...
from skq.gates.qutrit.base import QutritGate
...
class QutritXGate(QutritGate):
    ...
    """
    def __new__(cls):
        return super().__new__(cls, np.array([[0, 0, 1], 
                                              [1, 0, 0],
                                              [0, 1, 0]]))
...
class QutritYGate(QutritGate):
    ...
    """
    def __new__(cls):
        return super().__new__(cls, np.array([[0, 0, -1j], 
                                              [-1j, 0, 0], 
                                              [0, -1j, 0]]))
...
class QutritHGate(QutritGate):
    ...
    def __new__(cls):
        ...
        return super().__new__(cls, np.array([
            [1, 1, 1], 
            [1, omega, omega**2], 
            [1, omega**2, omega]
        ]) / np.sqrt(3))
