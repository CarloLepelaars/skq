...
def test_quantumlayer_single_multi_qubit():
...
def test_quantumlayer_single_multi_qubit_swap():
...
def test_quantumlayer_multiple_single_qubits():
    H_transformer = HTransformer(qubits=[0])
...
def test_quantumlayer_clashing_qubits():
    CX_transformer = CXTransformer(qubits=[0, 1])
...
def test_quantumlayer_invalid_transformer():
...
def test_superpositionlayer():
    layer = SuperpositionLayer(n_qubits=3)
    X = np.array([[1, 0, 0, 0, 0, 0, 0, 0]], dtype=complex)
    transformed_X = layer.transform(X)
    # Transformed should be valid statevector
    state = Statevector(transformed_X[0])
    # All elements in vector are equal
    assert np.all(state == state[0])
